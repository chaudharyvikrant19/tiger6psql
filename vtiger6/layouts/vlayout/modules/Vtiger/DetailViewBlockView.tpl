...
{strip}
	...
	<table class="table table-bordered equalSplit detailview-table">
			...
			{if !$FIELD_MODEL->isViewableInDetailView()}
			 ...
			 {if $FIELD_MODEL->get('uitype') eq "83"}
				...
				{if $tax.check_value eq 1}
					...
					{/if}
					<td class="fieldLabel">
					<label class='muted'>{vtranslate($tax.taxlabel, $MODULE)}(%)</label>
					</td>
					 <td class="fieldValue">
						 <span class="value">
							 {$tax.percentage}
		...
		{if $FIELD_MODEL_LIST|@count eq 1 and $FIELD_MODEL->get('uitype') neq "19" and $FIELD_MODEL->get('uitype') neq "20"}<td></td><td></td>{/if}
		{/foreach}
		</tr>
		<tbody>
	</table>
